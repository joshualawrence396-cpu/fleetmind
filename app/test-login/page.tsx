...
export default function TestLogin() {
  ...
  const testLogin = async () => {
    ...
    try {
      ...
      const data = await res.json()
      setResult({ type: 'login', data, status: res.status })
      
      if (res.ok) {
        // Test me endpoint
        const meRes = await fetch('/api/me')
        const meData = await meRes.json()
        setResult(prev => ({ ...prev, meData }))
      }
    } catch (error) {
      setResult({ type: 'error', error: error.message })
    }
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '28px', fontWeight: 'bold', marginBottom: '20px' }}>🔧 Debug Login</h1>
      ...
      {result && (
        <div style={{ padding: '20px', background: '#1e293b', color: '#e2e8f0', borderRadius: '8px', overflow: 'auto' }}>
          <h3 style={{ color: '#60a5fa', marginBottom: '10px' }}>Result:</h3>

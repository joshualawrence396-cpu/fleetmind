...
export function BlockchainVerification({ orderId }) {
  ...
  return (
    <div style={{ padding: '20px', background: 'white', borderRadius: '12px', marginTop: '20px' }}>
      <h3 style={{ fontSize: '18px', marginBottom: '15px' }}>🔗 Blockchain Verification</h3>
      
      <button 
        onClick={verifyDelivery} 
        disabled={verifying}
        style={{ padding: '10px 20px', background: '#6366f1', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', width: '100%' }}
      >
        {verifying ? 'Recording on Blockchain...' : '📦 Verify Delivery on Blockchain'}
      </button>
      
      {blockchainData && (
        <div style={{ marginTop: '20px', padding: '15px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #86efac' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '10px' }}>✅ Transaction Verified</div>

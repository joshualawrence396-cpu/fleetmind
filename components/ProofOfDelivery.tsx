...
export function ProofOfDelivery() {
  ...
  const [done, setDone] = useState<CompletedDelivery[]>([])

  useEffect(() => {
    fetch('/api/orders')
      .then((r) => r.json())
      .then((d: Order[]) =>
        setOrders((d || []).filter((o: Order) => o.status === 'IN_PROGRESS'))
      )
  ...
  const submitPOD = async () => {
    ...
    if (res.ok) {
      const newDone: CompletedDelivery[] = [
        ...done,
        {
          ...selected,
          note,
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          >
            ...
            {orders.length === 0 && (
              ...
              >
                No active deliveries. Orders must be IN_PROGRESS to submit POD.
              </div>
            )}
            ...
            {orders.map((o) => (
              <div
                ...
                style={{
                  ...
                  border: '2px solid',
                  borderColor:
                    selected?.id === o.id ? '#667eea' : '#e2e8f0',
                  cursor: 'pointer',
                  background:
                    selected?.id === o.id ? '#f5f3ff' : 'white',
                  transition: 'all 0.2s',
                }}
              >
          ...
          {selected && (
            ...
            >
              <h3 style={{ marginBottom: 15, fontWeight: 'bold' }}>
                Submit POD - {selected.orderNumber}
              </h3>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea
                  ...
                  placeholder="Left at door, signed by recipient, etc..."
                  style={{
                    width: '100%',
                    padding: 10,
                    border: '1px solid #e2e8f0',
        ...
        >
          <h3 style={{ marginBottom: 15, fontWeight: 'bold' }}>
            ✅ Completed Deliveries ({done.length})
          </h3>

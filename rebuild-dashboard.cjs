        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 10px" }}>
          {NAV.map(section => (
            <div key={section.label} style={{ marginBottom: 4 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#334155", letterSpacing: "1.5px", padding: "8px 8px 4px" }}>{section.label}</div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ height: 60, background: "rgba(13,21,38,0.95)", borderBottom: "1px solid rgba(99,102,241,0.08)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#475569" }}>Welcome back, {user?.name} — {time.toLocaleTimeString()}</div>
          </div>
          ...
          <button onClick={() => setShowNewOrder(true)} style={{ padding: "8px 16px", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", borderRadius: 9, color: "white", cursor: "pointer", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}><Plus size={14} /> New Order</button>
          ...
          {activeTab === "dashboard" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
                {[
                  { label: "Total Vehicles", value: stats.vehicles, sub: vehicles.filter((v: any) => v.status === "ON_ROUTE").length + " on route", color: "#6366f1", icon: <Truck size={20} color="#6366f1" /> },
                  { label: "Active Drivers", value: stats.drivers, sub: "Available now", color: "#10b981", icon: <Users size={20} color="#10b981" /> },
                  { label: "Total Orders", value: stats.orders, sub: stats.completed + " completed", color: "#f59e0b", icon: <Package size={20} color="#f59e0b" /> },
                  { label: "Success Rate", value: stats.rate + "%", sub: "R" + stats.revenue.toLocaleString() + " revenue", color: "#ec4899", icon: <BarChart2 size={20} color="#ec4899" /> },
                ].map(s => (
...
console.log("Dashboard completely rewritten! Length:", content.length);
